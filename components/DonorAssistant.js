...
export default function DonorAssistant({ campaignAddress }) {
  ...
  // Move all color hooks to top level - MUST be called unconditionally
  const panelBg = useColorModeValue("white", "gray.800");
  const replyBg = useColorModeValue("teal.50", "gray.700");
  const userBg = useColorModeValue("blue.50", "blue.900");
  const muted = useColorModeValue("gray.600", "gray.300");
  const conversationHistoryBg = useColorModeValue("gray.50", "gray.900");
